...
model = dict(
    type='RecognizerGCNVisual',
    backbone=dict(
        type='STGCN',
        graph_cfg=dict(layout='nturgb+d', mode='stgcn_spatial')),
    cls_head=dict(type='VisualHead', num_classes=60, in_channels=256))
...
data = dict(
    ...
    val=dict(type=dataset_type, ann_file=ann_file, pipeline=val_pipeline, split='xsub_train'),
...
# runtime settings
log_level = 'INFO'
work_dir = './work_dirs/stgcn/ntu60_xsub_LT_stgcn/exp_' + imb_factor + '/j'
