...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    def forward(self, imgs, label=None, epoch=None, split=None, return_loss=True, **kwargs):
        """Define the computation performed at every call."""
        if return_loss:
            if label is None:
                raise ValueError('Label should not be None.')
            return self.forward_train(imgs, label, epoch, split, **kwargs)

        return self.forward_test(imgs, **kwargs)

    def train_step(self, data_batch, optimizer, epoch, split, **kwargs):
        ...
        losses = self(**data_batch, epoch=epoch, split=split, return_loss=True)
