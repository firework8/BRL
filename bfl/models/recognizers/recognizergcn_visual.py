...
class RecognizerGCNVisual(BaseRecognizer):
    ...
    def forward_train(self, keypoint, label, epoch, split, **kwargs):
        ...
        loss = self.cls_head.loss(cls_score, gt_label, epoch, split)
        losses.update(loss)
    ...
    def forward_test(self, keypoint, label, **kwargs):
        ...
        gt_label = label.squeeze(-1)
        cls_score, save_x= self.cls_head(x)
        save_label = gt_label.data.cpu().numpy()
        ...
        return cls_score.data.cpu().numpy(), save_x, save_label
